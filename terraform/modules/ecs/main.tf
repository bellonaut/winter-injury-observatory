...
resource "aws_security_group" "ecs_tasks" {
  ...
  ingress {
    ...
    protocol        = "tcp"
    security_groups = [aws_security_group.alb.id]
    description     = "Dagster UI access from ALB"
  }
...
# Target Group for Dagster
resource "aws_lb_target_group" "dagster" {
  name        = "${var.project_name}-${var.environment}-dagster-tg"
  port        = 3000
  protocol    = "HTTP"
  vpc_id      = var.vpc_id
  target_type = "ip"
...
resource "aws_ecs_task_definition" "api" {
  ...
  container_definitions = jsonencode([
    {
      name  = "api"
      image = "${var.project_name}-${var.environment}-api:latest"
